    ...
    <div className="space-y-8 min-h-screen p-1 text-slate-100 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-300">Suivi des indicateurs de sinistralité, répartition des risques et performance financière en RDC.</p>
        </div>
        
        {/* Bouton Export ARCA */}
        <button 
          onClick={handleExportData}
          className="touch-target flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold shadow-md transition-all transform active:scale-95 border-b-2 border-slate-950 self-start sm:self-center"
        >
      ...
      <div className="admin-panel p-4 rounded-xl flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select 
            ...
            className="touch-target px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-xs font-semibold text-slate-100 focus:outline-none focus:ring-1 focus:ring-[#00A3E0]"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {financialKPIs.map((kpi, idx) => (
          <div key={idx} className="admin-panel p-6 rounded-2xl relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 admin-panel p-6 rounded-2xl flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-5">
              {branchDistribution.map((branch, idx) => (
                <div key={idx} className="space-y-2">
                  ...
                  <div className="text-[10px] text-slate-300 pl-7 flex items-center gap-1">
          ...
          <div className="text-[10px] text-slate-300 text-center font-semibold pt-4 mt-6 border-t border-white/10 flex items-center justify-center gap-1.5">
            <FaCalendarAlt /> Exercice fiscal en cours : {currentYear}
          </div>
        </div>

        {/* Tableau de Sinistralité Régionale */}
        <div className="lg:col-span-2 admin-panel p-6 rounded-2xl">
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-white/5 font-medium text-slate-200">
                {provinceStats.map((item, idx) => (
                  <tr key={idx}>
                    <td className="py-4 font-bold text-white">{item.province}</td>

...
export default function FinanceDashboard() {
    ...
    <div className="space-y-8 min-h-screen p-1 text-slate-100 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-300">Gestion de la compensation des devises transfrontalières, fixation du taux Forex interne et conciliation bancaire.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 admin-panel p-6 rounded-2xl space-y-4">
          ...
          <div className="flex gap-2">
            <div className="relative flex items-center flex-grow">
              <input 
                ...
                className="touch-target w-full pl-4 pr-12 py-2.5 rounded-xl border border-white/10 bg-white/5 font-mono font-black text-white text-sm focus:outline-none focus:ring-1 focus:ring-[#00A3E0]"
              />
              <span className="absolute right-4 text-xs font-bold text-slate-400 font-mono">CDF</span>
            </div>
            <button 
              onClick={handleUpdateRate}
              className="touch-target px-4 bg-[#00A3E0] hover:bg-[#0082B3] text-white rounded-xl text-xs font-bold transition-all border-b-2 border-[#006180] shrink-0"
            >
              Mettre à jour
            </button>
          </div>
        ...
        <div className="lg:col-span-2 admin-panel p-6 rounded-2xl flex flex-col justify-between">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {forexVolumes.map((vol, idx) => (
              <div key={idx} className="p-4 bg-white/5 rounded-xl border border-white/10 space-y-1">
                <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wide">{vol.currency}</span>
                <span className={`text-lg font-black block font-mono ${vol.color}`}>{vol.amount}</span>
                <span className="text-[10px] text-slate-300 font-semibold block pt-1 border-t border-white/10">Quote-part : {vol.share}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="admin-panel p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div>
 ...
 intègre un mécanisme de couverture automatique. Toutes les polices émises auprès de la diaspora en devises étrangères font l'objet d'un provisionnement réglementé indexé sur le dollar américain (USD) pour contrer l'inflation du marché congolais local.
        ...
        <div className="bg-white/5 p-5 rounded-xl border border-white/10 space-y-4">
          <h4 className="font-bold text-slate-200 text-xs uppercase tracking-wider flex items-center gap-1.5 border-b border-white/10 pb-2">
